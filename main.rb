...
helpers do
  ...
  def winner!(msg)
    @winner = "<strong>Congratulations, you win!</strong> #{msg}"
  ...
  def loser!(msg)
    @loser = "<strong>You lose.</strong> #{msg}"
    @show_hit_or_stay = false
    @play_again = true
    session[:player_pot] = session[:player_pot] - session[:player_bet]
...
post '/bet' do
  if params[:bet_amount].nil? || params[:bet_amount].to_i == 0
    ...
    @error = "Bets cannot be larger than what you currently have in your wallet. You currently have #{session[:player_pot]} left. Please bet again."
    halt erb(:bet)
  else
...
get '/game/dealer' do
  ...
  elsif dealer_total > DEALER_STAY || dealer_total = DEALER_STAY
    #dealer stays
